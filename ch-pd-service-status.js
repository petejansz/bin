...
{
    var outputStr = util.format( "-- Generated: %s", new Date() )
    outputStr += "-- " + sqlt.description + "\n"

    var sqlStatementTemplate = statements.join( "\n\n" )

    var doSQLStatements = false
    ...
    if ( doSQLStatements )
    {
        if ( program.of )
        {
            fs.writeFileSync( program.of, outputStr )
        }
        else
        {
