...
var authCode = await getOauth( oauthAxiosInstance, 'test50@yopmail.com', 'Password123', getOAutheResponseHandler )
...
async function getOauth( oauthAxiosInstance, username, password )
{
    ...
    return oauthAxiosInstance.post( '/api/v1/oauth/login', reqData )//.then( function ( response )
    // {
...
function getOAutheResponseHandler( response )
{
    ...
    // getLoginToken(oauthAxiosInstance, response.data[0].authCode, 'SolSet2ndChancePortal', lib1.caConstants.siteID)
