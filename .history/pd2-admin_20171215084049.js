...
async function main()
{
    ...
    {
        pd2admin.getPlayerId( program.username, program.host, program.port, getPlayerIdResponseHandler, getPlayerIdErrorHandler )
    }
    else if ( program.api === 'search' && program.host.match( /^cat1$|^cat2$|^localhost$|^pdadmin$/i ) )
    {
        var pdAdminSystem = createPdAdminSystem( program )
        var promised = util.promised(pd2admin.searchForPlayers)
        var response = await pd2admin.searchForPlayers( pdAdminSystem )//, searchPlayersResponseHandler )
