...
function processPlayers( playerIdList )
{
    ibmdb.open( dsn, function ( err, conn )
    {
        ...
        for ( i = 0; i < playerIdList.length; i++ )
        {
            ...
            updateCustomerContactsStmt.executeNonQuery( [playerId], function (err, rows) {if (err) console.err(err)})
            updateCustomerServicesStmt.executeNonQuery( [playerId], function (err, rows) {if (err) console.err(err)})
            updateCustomerProfileDataStmt.executeNonQuery( [playerId], function (err, rows) {if (err) console.err(err)})
            updateContractsStmt.executeNonQuery( [playerId], function (err, rows) {if (err) console.err(err)})
        }
