...
var Pd2Admin = ( function ()
{
    ...
    createNote = function( host, port, playerId, responseHandler )
    {
        var util = require( 'util' )
        var request = require( process.env.USERPROFILE + '/AppData/Roaming/npm/node_modules/request' )
        var lib1 = require( process.env.USERPROFILE + "/Documents/bin/lib1.js" )
        var restPath = '/california-admin-rest/api/v1/admin/players/%s/note'
        const url = util.format(
            '%s://%s:%s/california-admin-rest/api/v1/admin/players/%s/note',
            'http', host, ( port ? port : lib1.adminPort ), playerId )
        var options =
            {
                method: 'POST',
                url: url,
