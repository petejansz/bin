...
const oauthLogin = spawnSync( 'node', ['pd-oauth-login.js', '-h', program.hostname, '-u', program.username, '-p', program.password],
    {
        shell: true
    }
);
...
const oauthTokens = spawnSync( 'node', ['oauth-tokens.js', '-h', program.hostname, '-a', oauthLogin.stdout.toString().trim()],
    {
        shell: true
    }
);
